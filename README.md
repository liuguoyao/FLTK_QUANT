...
> **配置文件(config.ini)**:MySQL 连接参数从 `config.ini` 读取(由 inih 解析)。仓库提供 `config.ini.example` 模板(已入库);首次使用复制为 `config.ini` 并填入真实密码。`config.ini` 已在 `.gitignore` 中忽略(含密码,不提交)。`DataSourceConfig` 默认构造时自动按顺序查找 `config.ini` → `config.ini.example`,缺失的键回退到内置默认值。构建时会把配置拷到 exe 目录。
