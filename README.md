...
`third_party/` 下放三个第三方库,**都不入 git**(`.gitignore` 的 `/third_party` 已忽略),需手动 clone(见下文「获取源码」)。其中 FLTK 与 mariadb-connector-c 由 CMake 用 `FetchContent(SOURCE_DIR ...)` **纯本地**引入 —— 配置时不联网拉取。
...
> 三个目录都在 `.gitignore` 的 `/third_party` 规则下,不会被提交。
...
```
.
├── CMakeLists.txt              # 构建脚本:FetchContent(fltk/mariadb,纯本地) + fl_opdesk + datasource + dag_editor + test_datasource
├── src/
...
│   └── datasource.h/.cpp       # 数据源后端:IDataSource 抽象 + MysqlDataSource(libmariadb)
├── test/
...
- **`datasource`** —— `IDataSource` 抽象接口 + `MysqlDataSource`(libmariadb)实现,`mysql_real_escape_string` 转义防注入,utf8mb4 字符集。
