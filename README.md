...
| 节点 | 输入端口 | 输出端口 | 用途 |
|------|----------|----------|------|
| **公式输入** | (无) | 公式 | 因子数据源,作为 DAG 的起点 |
| **特征工程** | 特征公式 | 特征工程 | 接收因子公式,输出工程化特征 |
| **XGBoost模型** | 特征工程, 模型 | (内部参数) | 接收特征和模型,训练/预测 |
| **数据源** | (无) | 数据 | 原始数据源节点:DAG 起点。带 source 下拉框选择 `config.ini` 里已配置的命名数据源 |
...
`third_party/` 下放五个第三方库,**都不入 git**(`.gitignore` 的 `/third_party` 已忽略),需手动 clone(见下文「获取源码」)。其中 FLTK 与 mariadb-connector-c 由 CMake 用 `FetchContent(SOURCE_DIR ...)` **纯本地**引入 —— 配置时不联网拉取;spdlog 用 `add_subdirectory` 编译为静态库;inih 手动编为静态库。
...
| `third_party/inih/` | https://github.com/benhoyt/inih.git | **最新 master** | 手动编为静态库 `inih`(`ini.c` + `cpp/INIReader.cpp`),供 `datasource` 读 `config.ini` |
...
└── inih/                      # inih 本地 clone(INI 解析,最新 master)
```
...
> **配置文件(config.ini)**:由 inih 解析。含两类段:
> - `[mysql]` —— 默认连接配置(`DataSourceConfig` 兜底用,旧格式,向后兼容)。
> - `[source.<名称>]` —— **命名数据源**(可配多个),供画布上的**数据源节点**下拉选择。每个段含 `name`、`type`(mysql|sqlite)及连接字段。
>
> 首次使用:复制 `config.ini.example` 为 `config.ini` 并填入真实密码。`config.ini` 在 `.gitignore` 中忽略(含密码,不提交)。`DataSourceConfig` 默认构造按 `config.ini` → `config.ini.example` 查找 `[mysql]`。构建时会把配置拷到 exe 目录。
>
> **图形化添加数据源**:菜单「**数据源 → 配置数据源**」打开对话框,填 名称 / 类型(mysql|sqlite)/ 连接信息,保存即追加一个 `[source.<名称>]` 段到 `config.ini`。
>
> **数据源节点**:左侧面板「数据源」拖到画布,生成一个带 **source 下拉框**(列出 `config.ini` 里所有 `[source.*]` 名称)和「数据」输出端口的节点。它是 DAG 的起点(无输入端口)。
>
> ⚠️ SQLite 目前仅 **UI 占位**(下拉/对话框可选 sqlite,但后端实现尚未做)。

## 获取源码

克隆本项目后,需把五个第三方库放进 `third_party/`:
...
# 5) inih(INI 配置解析库)
git clone https://github.com/benhoyt/inih.git third_party/inih
```

然后配置 MySQL 连接:

```bash
cp config.ini.example config.ini   # Windows 下用 copy
# 编辑 config.ini 填入真实密码(该文件不入 git)
```

> 五个 `third_party/` 目录都在 `.gitignore` 的 `/third_party` 规则下;`config.ini` 也在 `.gitignore` 中,均不会被提交。
...
```
.
├── CMakeLists.txt              # 构建脚本:FetchContent(fltk/mariadb,纯本地) + fl_opdesk + spdlog + inih + datasource + dag_editor + test_datasource
├── config.ini.example         # MySQL 连接配置模板(入库);复制为 config.ini 填真实密码
├── src/
...
│   ├── datasource.h/.cpp       # 数据源后端:IDataSource 抽象 + MysqlDataSource(libmariadb)+ config.ini(inih)+ spdlog 日志
│   └── DsLogger.h/.cpp         # 共享日志器(spdlog,控制台 + 滚动文件)
├── test/
